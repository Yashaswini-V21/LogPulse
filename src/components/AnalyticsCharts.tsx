...
export function AnalyticsCharts({ isConnected }: AnalyticsChartsProps) {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      const now = new Date();
      let start: Date;
      if (selectedTimeRange === '15m') start = new Date(now.getTime() - 15 * 60 * 1000);
      else if (selectedTimeRange === '1h') start = new Date(now.getTime() - 60 * 60 * 1000);
      else if (selectedTimeRange === '6h') start = new Date(now.getTime() - 6 * 60 * 60 * 1000);
      else start = new Date(now.getTime() - 24 * 60 * 60 * 1000);

      const logsResult = await apiClient.query({}, start.toISOString(), now.toISOString(), 1000);
      
      // Group logs by minute/hour for time series
      ...
      logsResult.logs.forEach(log => {
        ...
        const bucket = new Date(Math.floor(t.getTime() / intervalMs) * intervalMs);
        const key = bucket.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
        if (!buckets[key]) buckets[key] = { time: key, errors: 0, warns: 0, info: 0, debug: 0, total: 0 };
        buckets[key].total++;
        if (log.level === 'error') buckets[key].errors++;
        else if (log.level === 'warn') buckets[key].warns++;
        else if (log.level === 'info') buckets[key].info++;
        else if (log.level === 'debug') buckets[key].debug++;
      });
      
      const data = Object.values(buckets).sort((a: any, b: any) => a.time.localeCompare(b.time));
      setVolumeData(data as any);
        ...
        setServiceHealth([
          {
            ...
            errorRate: health.ingestionRate || 0,
            avgLatency: 0,
            requestCount: health.chunksCount || 0,
          },
        ]);
      } catch {
        setServiceHealth([]);
